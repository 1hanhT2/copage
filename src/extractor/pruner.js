...
export function pruneElementTree(node, depth = 0, options = { maxDepth: 10, truncateListThreshold: 3 }) {
    ...
    if (tagName === "IMG") {
        ...
        if (/analytics|pixel|telemetry|doubleclick|bat\.bing/i.test(src))
            return null;
    }
    // Create clean clone
    ...
    for (let i = 0; i < el.attributes.length; i++) {
        ...
        if (name === "id") {
            if (attr.value.startsWith(":") || attr.value.length > 30)
                continue;
        }
    ...
    if (omittedCount > 0) {
        cleanEl.appendChild(document.createComment(` ... [${omittedCount} similar items omitted for brevity] ... `));
    }
    return cleanEl;
}
export function formatPrunedHtml(root) {
